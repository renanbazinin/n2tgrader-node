...
function findClosestName(target, candidates) {
    ...
    for (const original of candidates) {
        ...
        if (!best || d < best.dist)
            best = { name: candBase, dist: d };
    }
    if (best && best.dist <= 2)
        return best.name; // threshold
    return undefined;
}
// Helper to determine the test file needed for a specific project file
const loadAssignment = (fsys) => async function (file) {
...
async function main() {
    await yargs(hideBin(process.argv))
        .command("* <zipfile>", "Grade a Nand2Tetris submission zip", (yargs) => yargs.positional("zipfile", { type: "string", demandOption: true }), async (argv) => {
        const zipPath = path.resolve(argv.zipfile);
        ...
        for (const rel of solutionFiles) {
            ...
            if (!solutionMap.has(key))
                solutionMap.set(key, { files: {}, relDir: dir });
        ...
        function findSolutionForName(name) {
            ...
            const files = {};
            for (const rel of solutionFiles) {
                if (path.dirname(rel) !== dir)
                    continue;
                const fname = path.basename(rel);
                const nameNoExt = fname.replace(/\.[^.]+$/, '');
                if (nameNoExt.toLowerCase() !== lc)
                    continue;
            ...
            try {
                ...
                if (p.name === 'Fill') {
                    try {
                        ...
                        if (!candidateName)
                            throw new Error('No Fill.asm/hack found in archive');
                        ...
                        if (maybeT.error)
                            throw new Error('Failed to parse internal Fill TST');
                        const maybeTest = CPUTest.from(maybeT.ok ? maybeT.ok : maybeT, {
                            dir: '.',
                            doLoad: (async () => rom),
                            requireLoad: false,
                        });
                        ...
                        if (test.init)
                            await test.init();
                        if (test.load)
                            await test.load(path.basename(candidateName));
                        // helper to run N ticktocks and capture log
                        const runTicks = async (n) => {
                            for (let i = 0; i < n; i++) {
                                if (typeof test.ticktock === 'function')
                                    test.ticktock();
                                else if (typeof test.tick === 'function')
                                    await test.tick();
                                else
                        ...
                        await runTicks(FILL_TICKS);
                        const r3 = readRow();
                        const allZeros = (r) => r.length > 0 && r.every((v) => v === 0);
                        const allOnes = (r) => r.length > 0 && r.every((v) => v === -1);
                        ...
                        asmResults.push({ name: 'Fill', pass, out: JSON.stringify({ r1, r2, r3 }), cmp: cmp || '', diff: undefined, debug: { r1, r2, r3 } });
                        continue; // skip normal asm handling for Fill
                    }
                    catch (e) {
                        asmResults.push({ name: 'Fill', pass: false, out: `Interactive Fill error: ${e.message}` });
                        continue;
                    }
                }
                // Parse test
                ...
                if (test.with === undefined && test.value) {
                    // assume Ok wrapper
                ...
                try {
                    ...
                    asmResults.push({ name: p.name, pass: false, out: `Test run error: ${e.message}` });
                    continue;
                }
                const out = test.log();
                ...
                asmResults.push({ name: p.name, pass: false, out: e.message });
            }
        }
        // Debug: print asmResults summary
        try {
            console.log('[DEBUG] asmResults:', asmResults.map((r) => ({ name: r.name, pass: r.pass, hasCmp: !!r.cmp, diffLines: r.diff ? r.diff.split('\n').length : 0 })));
        }
        catch (_e) { }
        ...
        for (const t of results) {
            if (!seenResults.has(t.name))
                seenResults.set(t.name, t);
            else {
        ...
        try {
            console.log('[DEBUG] dedupedResults:', JSON.stringify(dedupedResults, null, 2));
        }
        catch (_e) { }
        ...
        try {
            ...
            for (const d of dedupedResults)
                resMap.set(String(d.name).toLowerCase(), d);
            const resultsByFile = {};
            for (const [baseLower, meta] of solutionMap.entries()) {
                ...
                let expectedFromSolution = null;
                try {
                    if (meta.files['.cmp']) {
                        const full = path.join(solutionsRoot, meta.files['.cmp']);
                        if (fs.existsSync(full))
                            expectedFromSolution = fs.readFileSync(full, 'utf8');
                    }
                }
                catch (_e) {
                    expectedFromSolution = null;
                }
                if (!found) {
            ...
            const safeRaw = dedupedResults.map((r) => ({ name: r.name, pass: !!r.pass, out: r.out || null, cmp: r.cmp || null, diff: r.diff || null, debug: r.debug || null }));
            const resultObj = {
            ...
            console.error('[DEBUG] summary error:', _err.message || _err);
            try {
            ...
            if (!test.pass) {
                ...
                if (test.cmp) {
                    ...
                    if (test.diff) {
                        ...
                        const expLines = test.cmp.split(/\r?\n/).map((l) => '      - ' + l).join('\n');
